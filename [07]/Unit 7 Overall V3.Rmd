...
NYTIMES_KEY = "D7jxG7fmsx7w2AE0qGSY1G3DfKZRzYFt" #Your Key Here … get from NTY API website
...
Pnews_word = function(key_word, trainingSet, alphaLaplace = 1, betaLaplace = 1) # alpha and beta are for laplace smoothing
{
  ...
  pKWGivenNews = (length(str_which(NewsGroup$response.docs.headline.main,regex(str_c("\\b",key_word,"\\b",sep=""),ignore_case = TRUE))))/(dim(NewsGroup)[1])
  pKWGivenOther = (length(str_which(OtherGroup$response.docs.headline.main,regex(str_c("\\b",key_word,"\\b",sep=""),ignore_case = TRUE))))/(dim(OtherGroup)[1])
